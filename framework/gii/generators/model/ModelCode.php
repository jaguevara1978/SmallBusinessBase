...
class ModelCode extends CCodeModel
{
	...
	public function generateRules($table)
	{
		...
		foreach($table->columns as $column) {
			...
			//else $r=!$column->allowNull && $column->defaultValue===null;
			if(!$column->allowNull && !$column->autoIncrement) $required[]=$column->name;
			if(strpos(strtolower($column->dbType),'decimal') !== false)
			...
			//$column->type==='string' &&
			if($column->size>0) $length[$column->size][]=$column->name;

			if(!$column->isPrimaryKey && !$r) $safe[]=$column->name;
		}

		if($primaryKey!==array()) $rules[]="array('".implode(',',$primaryKey)."', 'UnqAttrValidate', 'with'=>'".implode(',',$primaryKey)."')";
		...
		if($numerical!==array()) $rules[]="array('".implode(',',$numerical)."','numerical','min'=>0)";
		if($length!==array()) {
	...
	public function class2table($name) {
		$ret='';
