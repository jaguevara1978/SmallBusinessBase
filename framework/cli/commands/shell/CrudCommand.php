...
EOD;
	...
	public function generateActiveField($modelClass,$column) {
		...
		if($column->type==='boolean')
		...
		else if(stripos($column->dbType,'text')!==false || $size > 90)
			return "\$form->textAreaRow(\$model,'{$column->name}', array(".$span.", 'rows'=>1, 'size'=>$size,'maxlength'=>$maxLength))";
		else if($column->type==='date')
			return "<?php echo Utilities::formDatePicker(\$form, \$model,'{$column->name}');?>";
		else {
			if(preg_match('/^(password|pass|passwd|passcode)$/i',$column->name))
			...
			if($column->type!=='string' || $column->size===null)
				return "\$form->{$inputField}(\$model,'{$column->name}', array(".$span."))";
			else {
				if(($size=$maxLength=$column->size)>60) $size=60;
				return "\$form->{$inputField}(\$model,'{$column->name}',array(".$span.", 'size'=>$size,'maxlength'=>$maxLength))";
			}
		}
	}
