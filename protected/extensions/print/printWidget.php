 ...
 class printWidget extends CWidget
{
    ...
    */
    ...
    private function showPrintLink() {
    	$imgUrl = Yii::app()->getAssetManager()->publish(dirname(__FILE__) . DIRECTORY_SEPARATOR . 'media').'/printer.png';
    	if(!$this->text) {
    		$text="&nbsp;".Yii::t('app', 'Print ');
    		$this->text=Yii::t('app', 'Print ');
    	} else $text="&nbsp;".$this->text;
    	$img = CHtml::image($imgUrl,$text, array('title'=>$this->text));
    	echo CHtml::link($img . $text, "", array_merge($this->htmlOptions, 
    			array(
    				'title'=>$this->text,
    				'id' => 'img_link',
    				'onclick'=>'return processPrint();',
    				'class'=>'img_link',
    ...
    public function init() {
    	if(!$this->cssPath)
    		$this->cssPath=dirname(__FILE__) . DIRECTORY_SEPARATOR . 'media';
    	$this->showPrintLink();
        self::$count++;
        if (self::$count == 1) {
            Yii::app()->clientScript->registerCssFile(Yii::app()->getAssetManager()->publish($this->cssPath.'/'.$this->cssFile));
