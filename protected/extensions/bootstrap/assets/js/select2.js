...
(function ($, undefined) {
    ...
    AbstractSelect2 = clazz(Object, {
        ...
        /**
         * Get the desired width for the container element.  This is
         * derived first from option `height` passed to select2, then
         * the inline 'style' on the original element, and finally
         * falls back to the jQuery calculated element width.
         */
        // abstract
...
//                var style, attrs, matches, i, l;
//
//                if (this.opts.height === "off") {
//                    return null;
//                } else if (this.opts.height === "element"){
//                    return this.opts.element.outerHeight(false) === 0 ? 'auto' : this.opts.element.outerHeight(false) + 'px';
//                } else if (this.opts.height === "copy" || this.opts.height === "resolve") {
//                    // check if there is inline style on the element that contains width
//                    style = this.opts.element.attr('style');
//                    if (style !== undefined) {
//                        attrs = style.split(';');
//                        for (i = 0, l = attrs.length; i < l; i = i + 1) {
//                            matches = attrs[i].replace(/\s/g, '')
//                                .match(/height:(([-+]?([0-9]*\.)?[0-9]+)(px|em|ex|%|in|cm|mm|pt|pc))/);
//                            if (matches !== null && matches.length >= 1)
//                                return matches[1];
//                        }
//                    }
//
//                    if (this.opts.height === "resolve") {
//                        // next check if css('width') can resolve a width that is percent based, this is sometimes possible
//                        // when attached to input type=hidden or elements hidden via css
//                        style = this.opts.element.css('height');
//                        if (style.indexOf("%") > 0) return style;
//
//                        // finally, fallback on the calculated width of the element
//                        return (this.opts.element.outerHeight(false) === 0 ? 'auto' : this.opts.element.outerHeight(false) + 'px');
...
//
//                    return null;
//                } else if ($.isFunction(this.opts.height)) {
...
//            };
//
//            var height = resolveContainerHeight.call(this);
        	var height = '30px'; 
            if (height !== null) {
