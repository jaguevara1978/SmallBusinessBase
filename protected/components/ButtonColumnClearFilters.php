<?php
/**
* ButtonColumnClearFilters class file.

* The ButtonColumnClearFilters extension adds up some functionality to the default
...
* add 'import' => 'application.components.ButtonColumnClearFilters', [...] to your
...
*
* if (intval(Yii::app()->request->getParam('clearFilters'))==1) {
*    ButtonColumnClearFilters::clearFilters($controller,$model);
* }
*
*
...
class ButtonColumnClearFilters extends CButtonColumn {
    ...
    public function init()
    {
        ...
        if (!empty($this->imageUrl)) {
            ...
            $_imageUrl=Yii::app()->request->baseUrl.Yii::app()->params['General_BaseImageDirectory'].'ClearFilter.png';
        }
        ...
        if (!empty($this->label)) {
            ...
            $_label=Yii::t('app','general.label.clearFilters');
        }
...
HTMLEND;
    ...
    public static function clearFilters($controller,$model,$redirect=true) {
        $model->unsetAttributes();
        try {
            if (ButtonColumnClearFilters::modelUsesBehavior($model,'RememberFilters')) {
                $model->unsetFilters();
            }
        }
        catch (Exception $e) {
