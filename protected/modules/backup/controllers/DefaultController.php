...
class DefaultController extends Controller {
	...
	public function EndBackup($addcheck=true) {
		...
		if ( $addcheck ) fwrite($this->fp,'COMMIT;'.PHP_EOL);
		fwrite ( $this->fp, '-- -------------------------------------------'.PHP_EOL );
	...
	public function actionBatchDelete() {
		...
		if($request->getIsPostRequest()){
            ...
            foreach ($ids as $id) {
				$this->updateMenuItems();
				if (isset($id)) {
					$sqlFile=$this->path.$id;
					if (file_exists($sqlFile)) unlink($sqlFile);
					$successCount++;
				} else {
					$errors.=Yii::t('app', 'message.ErrorOnFileDeletion').' '.$id.'\n';
					$failureCount++;
				}
			}
	...
	public function actionIndex() {
		...
		foreach ( $list as $id=>$filename ) {
			...
			$columns['create_time']=date( "l > d-M-y > H:i:s", filectime($path .$filename) );
			$dataArray[]=$columns;
		}
	...
	protected function updateMenuItems($model=null) {
		...
		switch( $this->action->id) {
			case 'restore':
				$this->menu[]=array('label'=>Yii::t('app', 'general.button.ViewSite') , 'url'=>Yii::app()->HomeUrl);
				break;
			case 'create':
				$this->menu[]=array('label'=>Yii::t('app', 'general.button.List').' '.Yii::t('app', 'general.label.Backups'), 'url'=>array('index'));
				break;
			case 'upload':
				$this->menu[]=array('label'=>Yii::t('app', 'general.button.Create').' '.Yii::t('app', 'general.label.Backups'), 'url'=>array('create'));
				break;
			default:
				//$this->menu[]=array('label'=>Yii::t('app', 'general.button.List').' '.Yii::t('app', 'general.label.Backups'), 'url'=>array('index'));
				$this->menu[]=array('label'=>Yii::t('app', 'general.button.Create').' '.Yii::t('app', 'general.label.Backups'), 'url'=>array('create'));
				$this->menu[]=array('label'=>Yii::t('app', 'general.button.Upload').' '.Yii::t('app', 'general.label.Backups'), 'url'=>array('upload'));
				//$this->menu[]=array('label'=>Yii::t('app', 'general.button.Restore').' '.Yii::t('app', 'general.label.Backup') , 'url'=>array('restore'));
				//$this->menu[]=array('label'=>Yii::t('app', 'general.button.CleanDatabase') , 'url'=>array('clean'));
				$this->menu[]=array('label'=>Yii::t('app', 'general.button.ViewSite'), 'url'=>Yii::app()->HomeUrl);
				break;
		}
	}
